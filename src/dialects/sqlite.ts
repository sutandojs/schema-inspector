...
export default class SQLite implements SchemaInspector {
  ...
  async columns(table?: string): Promise<{ table: string; column: string }[]> {
    if (table) {
      const columns: RawColumn[] = await this.connection.raw(
        `PRAGMA table_xinfo(??)`,
        [table]
      );
  ...
  async columnInfo(table?: string, column?: string) {
    const getColumnsForTable = async (table: string): Promise<Column[]> => {
      ...
      const foreignKeys: { table: string; from: string; to: string }[] = await this.connection.raw(`PRAGMA foreign_key_list(??)`, [table]);

      const indexList: { name: string; unique: boolean }[] = await this.connection.raw(`PRAGMA index_list(??)`, [table]);

      const indexInfoList: { seqno: number; cid: number; name: string }[][] = await Promise.all(
        indexList.map((index) =>
          this.connection.raw(
            `PRAGMA index_info(??)`,
            [index.name]
  ...
  async foreignKeys(table?: string): Promise<ForeignKey[]> {
    if (table) {
      const keys = await this.connection.raw(`PRAGMA foreign_key_list(??)`, [table]);
