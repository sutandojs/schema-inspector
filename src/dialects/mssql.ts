...
export default class MSSQL implements SchemaInspector {
  ...
  async tables() {
    const records = await this.connection
      ...
      .where({
        ...
        TABLE_CATALOG: this.connection.connector.client.database(),
  ...
  async columnInfo<T>(table?: string, column?: string) {
    const dbName = this.connection.connector.client.database();

    const query = this.connection
      // @ts-ignore
      .table(this.connection.raw(`??.[sys].[columns] [c]`, [dbName]))
      .select(
        this.connection.raw(`
        [o].[name] AS [table],
        [c].[name] AS [name],
  ...
  async foreignKeys(table?: string) {
    const result: ForeignKey[] = await this.connection.raw(
      `
      SELECT
