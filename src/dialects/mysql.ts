...
export default class MySQL implements SchemaInspector {
  ...
  async tables() {
    const records = await this.connection
      ...
      .where({
        ...
        TABLE_SCHEMA: this.connection.connector.client.database(),
      })
  ...
  async tableInfo<T>(table?: string) {
    ...
    const records: RawTable[] = await query.get();
  ...
  async columns(table?: string) {
    const query = this.connection
      ...
      .where({ TABLE_SCHEMA: this.connection.connector.client.database() });

    if (table) {
      query.andWhere({ TABLE_NAME: table });
    }

    const records = await query.get<{ TABLE_NAME: string; COLUMN_NAME: string }>();
  ...
  async columnInfo<T>(table?: string, column?: string) {
    const query = this.connection
      .table('INFORMATION_SCHEMA.COLUMNS as c')
      .select(
        'c.TABLE_NAME',
        'c.COLUMN_NAME',
        'c.COLUMN_DEFAULT',
      ...
      .where({
        'c.TABLE_SCHEMA': this.connection.connector.client.database(),
      });
    ...
    const records: RawColumn[] = await query.get();
  ...
  async foreignKeys(table?: string) {
    const query = this.connection
      .table('information_schema.referential_constraints AS rc')
      .select(
      ...
      .where({
        'rc.CONSTRAINT_SCHEMA': this.connection.connector.client.database(),
      });
    if (table) {
      query.andWhere({ 'rc.TABLE_NAME': table });
    }
    const result: ForeignKey[] = await query.get();
    return result;
  }
}
