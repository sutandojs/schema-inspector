...
export default class Postgres implements SchemaInspector {
  connection: AnyQueryBuilder;
  schema: string;
  explodedSchema: string[];

  constructor(connection: AnyQueryBuilder) {
    this.connection = connection;
    const config = connection.connector.client.config;
  ...
  async tables() {
    const schemaIn = this.explodedSchema.map(
      (schemaName) => `${this.connection.raw('?', [schemaName])}::regnamespace`
    );
  ...
  async columnInfo<T>(table?: string, column?: string) {
    ...
    const versionResponse = await this.connection.raw(`SHOW server_version`);
    ...
    const [columns, constraints]: [{ rows: RawColumn[] }, { rows: Constraint[] }] = await Promise.all([
      connection.raw(
        `
        SELECT
          att.attname AS name,
          ...
          CASE
            WHEN t.typtype = 'e' THEN string_agg(e.enumlabel, '|' ORDER BY e.enumsortorder)
            ELSE NULL
          END AS enum_values
        FROM
          pg_attribute att
        ...
        WHERE
          ...
          AND NOT att.attisdropped
        GROUP BY
          att.attname, rel.relname, rel.relnamespace, att.atttypid, att.attnotnull, 
          att.atttypmod, des.description, t.typtype, att.attgenerated, ad.adbin, ad.adrelid, att.attnum
    ...
    const parsedColumms: Column[] = columns.rows.map((col): Column => {
      ...
      return {
        ...
        enum_values: this.parseEnumValues(col.enum_values),
